...
import { AppBar , Toolbar , Typography ,Container ,MenuItem, Select ,InputLabel, Box , Checkbox,FormControlLabel,Button
,ListItem,ListItemText,List,Divider} from "@mui/material";
...
export let schema = yup.object().shape({
  ...
  email:yup.string().
  when('subscriptionType',(["subscriptionType"],{
    is:(subscriptionType)=>{
    return subscriptionType === "Email"},
    then:schema=>yup.string().required("Email is required")
...
//  })
,
 phone_number:yup.string().when('subscriptionType',(['subscriptionType'],{
  is : (subscriptionType) =>{
    return subscriptionType === "SMS"
  },
  then : schema =>yup.string().required("PhoneNumber is required")
//   .test("phoneNumber","Enter valid Phonenumber",val=>{
//     let regex_phno= /^\(?(\d{3})\)?[- ]?(\d{3})[- ]?(\d{4})$/;
//     if(regex_phno.test(val))
//     return true
//   }),
//   otherwise:schema=>schema
  }))
,
network:yup.string().when('subscriptionType',(['subscriptionType'],{
  is:(subscriptionType)=>{
    return subscriptionType === "SMS"
  },
  then:()=>yup.string().test("network","Select Network---",
  function(value){
    return value !=="Select Network---"
  }).required("Network is required")
})),
subscriptionType:yup.string().test("subscriptionType","Select Subscription Type",
function(value){
...
const Subscribe = () =>
{const { register,handleSubmit,watch,formState,control} = useForm({mode:"all", resolver :yupResolver(schema),
defaultValues:{
  ...
  setMaintainChecked([])
   ...
   useEffect(()=>{
    ...
    console.log("sub",subCategoryList)
      ...
      console.log(watch('subscriptionType'))
useEffect( ( )=>{
...
const callCreateSubscriber = async(payload) =>{
  try{
   ...
   setSnackBarConfig({open:true,message:'Added successfully, you will receive confirmation email/message.',severity:"success"})
  ...
  return  <div className="status">
     ...
     {data.length > 0 && <Container sx={{ marginTop: "120px" }}>
  ...
  <Grid container spacing={2}>
    ...
    <Controller control={control}
        render={({ formState, fieldState:{error} ,field:{onChange , onBlur,value}}) => {
            return <TextField label="Enter FirstName" id="first_name"
          onChange={onChange}
          onBlur={onBlur}
          error={error} 
    ...
    <Controller control={control}
        render={({ formState, fieldState:{error} ,field:{onChange , onBlur,value}}) => {
            return <TextField label="Enter LastName" id="last_name"
          onChange={onChange}
    ...
    <Grid item md={12}>
      ...
      fieldState:{error}})=>{
   return (<Select id="subscriptionType"
   name="subscriptionType"
   onBlur={onBlur}
   onChange={onChange}
   error={error}
   ...
   >
    ...
    return <MenuItem key={item} value={item}>{item}</MenuItem>})}
     ...
     <Grid item md={12}>
    {subscriptionValue === "Email" &&<Controller control={control}
        render={({ formState, fieldState:{error} ,field:{onChange , onBlur,value}}) => {
            return <TextField label=" Enter Email" id="email"
          onChange={onChange}
          onBlur={onBlur}
          error={error}
          // sx={{ width:"300px"}}
      ...
      />            }
      {
        subscriptionValue === "SMS" && <Controller control={control}
        render={({ formState, fieldState:{error} ,field:{onChange , onBlur,value}}) => {
            return <TextField label="Enter PhoneNumber" id="phone_number"
          onChange={onChange}
          onBlur={onBlur}
          error={error}
          // sx={{ width:"300px"}}
    ...
    {subscriptionValue === "Email" && <label style={{ fontSize: 10, color: "grey" }}>
                      This email address will be subscribed to future
                      notifications
                    </label>}
                    {subscriptionValue === "SMS" && <label style={{ fontSize: 10, color: "grey" }}>
                        This Phone number will be subscribed to future
                        notifications
                      </label>}
                      </div>
    </Grid>
    {subscriptionValue === "SMS" && <Grid item md={12}>
    
      <Controller control ={control} 
      name="network"
      render= {({field:{onChange , onBlur , value},
      fieldState:{error}})=>{
   return (<Select id="network"
   name="network"
   ...
   >
    ...
    <label
                  ...
                  <Grid item md={6}>
                  <FormControlLabel sx={{ paddingRight:"50px",marginBottom:"5px"}}
                    control={
                      <Checkbox
