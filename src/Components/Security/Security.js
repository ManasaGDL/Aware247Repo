...
const Security = ()=>{
 ...
 useEffect(()=>{
    ...
    const columns=[{
        ...
        flex:0.8,
        valueGetter:(params)=>{
          if(typeof params.row.modifieduser?.first_name!=="undefined" && typeof params.row.modifieduser?.last_name !=="undefined")
            return params.row.modifieduser?.first_name+" "+params.row.modifieduser?.last_name
            else return '-----'
        }},
        {
            ...
            type:"actions",
            getActions:(params)=>[
                <GridActionsCellItem label="Update BusinessUnit"
                 icon={<UpdateIcon/>}
                onClick={()=>
                  { 
                    setbusinessunitvalue(params?.row?.businessunit_name)
                    setOpenDialog({open:true , action:"Edit",id:params.row.businessunit_id })}}
                showInMenu
                />,
                <GridActionsCellItem label={params.row.is_active=== true?"Deactivate BusinessUnit":"Activate BusinessUnit"}
                ...
                onClick={()=>{
                  setOpenDeleteDialog({open:true, id:params.row.businessunit_id, name:params.row.businessunit_name,action:"Delete"
                  });
                }}
                showInMenu/>
            ]
  ...
  color:"grey"
    ...
    const createBusinessUnit =async(val)=>{
      ...
      {   
    const response =await api.createNewBusinessUnit({"businessunit_name":val});
      }
      else{
 ...
 window.location.reload(true);
    ...
    const deleteBusinessUnit =async( )=>{
      ...
      try{
        ...
        {
   const response = await api.updateBusinessUnit(openDeleteDialog.id,{"is_active":0});
        }
   ...
   setOpenDeleteDialog({open:false,action:''})
    ...
    <LoadingPanel ></LoadingPanel>
        ...
        <StyledDataGrid columns={columns} rows={data}
      ...
      //      }
        ...
        <Dialog open={openDeleteDialog.open} onClose={()=>setOpenDeleteDialog({open:false})}>
          ...
          <DialogContent>
            Do you really want to  {openDeleteDialog.action} this BusinessUnit - <b>{openDeleteDialog.name}</b>
          </DialogContent>
          <DialogActions>
